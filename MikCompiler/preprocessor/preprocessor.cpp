...
void Preprocessor::preprocess(string file_contents, fs::path file_path) {
    ...
    if (std::count(file_path.string().begin(), file_path.string().end(), '/') ==
        0) {
    ...
    file_path =
        file_path.string().substr(0, file_path.string().find_last_of('/'));
    ...
    for (string line; getline(contents, line);) {
        ...
        if (regex_match(line, valid_local_include_regexp)) {
            ...
            if (file) {
                ...
                throw_error(ERROR_OPENING_YOINKED_FILE, line, ln_number,
                            org_path);
            }
