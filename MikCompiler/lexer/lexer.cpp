  ...
  for (; this->current_character != '\0' &&
         ((find(LEGAL_CHARACTERS_IN_ID.begin(), LEGAL_CHARACTERS_IN_ID.end(),
          ...
          (find(LEGAL_CHARACTERS_IN_NUMBER.begin(),
                ...
                this->current_character) != LEGAL_CHARACTERS_IN_NUMBER.end()));
       this->advance())
    res += this->current_character;
  ...
  if (hex_prefix) {
    ...
    for (;
         ...
         ((find(LEGAL_HEXADECIMAL_CHARACTERS.begin(),
                LEGAL_HEXADECIMAL_CHARACTERS.end(), this->current_character) !=
    ...
    for (;
         this->current_character != '\0' &&
         ((find(LEGAL_BINARY_CHARACTERS.begin(), LEGAL_BINARY_CHARACTERS.end(),
                this->current_character) != LEGAL_BINARY_CHARACTERS.end()) ||
          this->current_character == '.');
         this->advance()) {
      if (this->current_character == '.')
        throw_error(UNEXPECTED_DECIMAL_POINT, *this);
    ...
    // check if characters within legal characters for any base 10 number and
    // append
    for (; this->current_character != '\0' &&
           ((find(LEGAL_CHARACTERS_IN_NUMBER.begin(),
  ...
  while (this->current_character != '\0') {
      ...
      */
      ...
      if (this->tokens[this->tokens.size() - 1].value == "file") {
        ...
        if (this->current_character == '(') {
          ...
          // push new file to the positions stack. Will be popped if @fend
          // occurs
          ...
          if (this->current_character != ')') {
            throw_error(CLOSING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE,
                        *this);
          }
        } else {
          throw_error(OPENING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE,
                      *this);
        }
    ...
    case '0': {
      ...
      if (this->current_character == 'x') {
        ...
        if (find(LEGAL_BINARY_CHARACTERS.begin(), LEGAL_BINARY_CHARACTERS.end(),
                 this->current_character) == LEGAL_BINARY_CHARACTERS.end()) {
          this->current_index--;
    ...
    default: {
      if ((find(LEGAL_CHARACTERS_IN_ID.begin(), LEGAL_CHARACTERS_IN_ID.end(),
                this->current_character) != LEGAL_CHARACTERS_IN_ID.end())) {
        // ID
        this->lex_id_token();
