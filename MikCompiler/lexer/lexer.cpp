...
void Lexer::advance() {
    this->current_index++;
    this->current_character = (this->current_index) >= this->text.size() ? '\0' : this->text[this->current_index];
}
...
void Lexer::lex_string_token() {
    ...
    for(;this->current_character != '\0' && this->current_character != '"'; this->advance()) res += this->current_character;
...
void Lexer::lex_id_token() {
    ...
    for (; this->current_character != '\0' && ((find(LEGAL_CHARACTERS_IN_ID.begin(), LEGAL_CHARACTERS_IN_ID.end(), this->current_character) != LEGAL_CHARACTERS_IN_ID.end()) || (find(LEGAL_CHARACTERS_IN_NUMBER.begin(), LEGAL_CHARACTERS_IN_NUMBER.end(), this->current_character) != LEGAL_CHARACTERS_IN_NUMBER.end())); this->advance()) res += this->current_character;
...
void Lexer::lex_number_token(bool hex_prefix=false, bool bin_prefix=false) {
    ...
    if (hex_prefix) {
        // hex number
        // check if characters within legal characters for any hex number and append
        for(; this->current_character != '\0' && ((find(LEGAL_HEXADECIMAL_CHARACTERS.begin(), LEGAL_HEXADECIMAL_CHARACTERS.end(), this->current_character) != LEGAL_HEXADECIMAL_CHARACTERS.end()) || this->current_character == '.'); this->advance()) {
            if (this->current_character == '.') throw_error(UNEXPECTED_DECIMAL_POINT, *this);
        ...
        // check if characters within legal characters for any binary number and append
        for(; this->current_character != '\0' && ((find(LEGAL_BINARY_CHARACTERS.begin(), LEGAL_BINARY_CHARACTERS.end(), this->current_character) != LEGAL_BINARY_CHARACTERS.end()) || this->current_character == '.'); this->advance()) {
            if (this->current_character == '.') throw_error(UNEXPECTED_DECIMAL_POINT, *this);
            res += this->current_character;
        }
        ...
        // check if characters within legal characters for any base 10 number and append
        for(; this->current_character != '\0' && ((find(LEGAL_CHARACTERS_IN_NUMBER.begin(), LEGAL_CHARACTERS_IN_NUMBER.end(), this->current_character) != LEGAL_CHARACTERS_IN_NUMBER.end()) || this->current_character == '.'); this->advance()) {
            if (this->current_character == '.') {
                if (decimal_point_occured) throw_error(UNEXPECTED_DECIMAL_POINT, *this);
                else {
                    token_type = TT_FLOAT;
...
vector<Token> Lexer::lex(void) {
    while (this->current_character != '\0') {
        ...
        switch (current_character) {
            ...
            case '/':
                {
                    ...
                    if (current_character == '=')
                        ...
                        if (current_character == '\0') throw_error(UNTERMINATED_MULTILINE_COMMENT, *this);
                    ...
                    */ 
                    ...
                    if (this->tokens[this->tokens.size()-1].value == "file") {
                        ...
                        if (this->current_character == '(') {
                            ...
                            // push new file to the positions stack. Will be popped if @fend occurs
                            this->positions_in_files.push(pair<string, int>(this->tokens[this->tokens.size()-1].value, 1));
                            this->tokens.pop_back();

                            if (this->current_character != ')') {
                                throw_error(CLOSING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE, *this);
                            }
                        } else {
                            throw_error(OPENING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE, *this);
                        }
            ...
            case '0':
                {
                    ...
                    if (this->current_character == 'x') {
                        ...
                        if (find(LEGAL_HEXADECIMAL_CHARACTERS.begin(), LEGAL_HEXADECIMAL_CHARACTERS.end(), this->current_character) == LEGAL_HEXADECIMAL_CHARACTERS.end()) {
                            this->current_index--;
                        ...
                        if (find(LEGAL_BINARY_CHARACTERS.begin(), LEGAL_BINARY_CHARACTERS.end(), this->current_character) == LEGAL_BINARY_CHARACTERS.end()) {
                            this->current_index--;
            ...
            default: 
                {
                    if ((find(LEGAL_CHARACTERS_IN_ID.begin(), LEGAL_CHARACTERS_IN_ID.end(), this->current_character) != LEGAL_CHARACTERS_IN_ID.end())) {
                        // ID
                        this->lex_id_token();
                    } else if ((find(LEGAL_CHARACTERS_IN_NUMBER.begin(), LEGAL_CHARACTERS_IN_NUMBER.end(), this->current_character) != LEGAL_CHARACTERS_IN_NUMBER.end())) {
                        // Number
                        this->lex_number_token();
                    } else 
