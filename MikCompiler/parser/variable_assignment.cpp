...
all of those can be followed by:
    ... name ...{roll, <expression_lower_bound>, <expression_upper_bound>} = <expression>
    ... name ...{keep, <expression_lower_bound>, <expression_upper_bound>} = <expression>
on arrays this will simply apply to all variables in that array
*/
VariableAssignment Parser::parse_variable_assignment() {
    ...
    // or a bound
    if (this->current_token.token_type != TT_ASSIGNMENT && this->current_token.token_type != TT_OPENING_BRACKET && this->current_token.token_type != TT_OPENING_BRACE) {
        throw_error(EXPECTED_BOUND_OR_ARRAY_DEFINITION_OR_ASSIGNMENT_AFTER_VARIABLE_NAME, *this);
    }
