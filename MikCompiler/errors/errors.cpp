...
void throw_error(int err_code, Lexer state) {
    ...
    case UNEXPECTED_TOKEN: {
        cout << RED << "\n\r[ERROR " << to_string(UNEXPECTED_TOKEN)
             << "] An unexpected token (" << state.current_character
             << ") was found:\n"
             << endl;
    ...
    case UNTERMINATED_MULTILINE_COMMENT: {
        cout << RED << "\n\r[ERROR "
             << to_string(UNTERMINATED_MULTILINE_COMMENT)
             << "] A multiline comment was started but never terminated:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << UNTERMINATED_MULTILINE_COMMENT_LINK << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;

    case UNTERMINATED_STRING: {
        cout << RED << "\n\r[ERROR " << to_string(UNTERMINATED_STRING)
             << "] A string was started but not terminated:\n";
        print_error_location(state);
        cout << GREEN
    ...
    case UNEXPECTED_DECIMAL_POINT: {
        cout << RED << "\n\r[ERROR " << to_string(UNEXPECTED_DECIMAL_POINT)
             << "] An unexpected decimal point was found:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << UNEXPECTED_DECIMAL_POINT_LINK << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;

    case HEX_LITERAL_EXPECTED: {
        cout << RED << "\n\r[ERROR " << to_string(HEX_LITERAL_EXPECTED)
             << "] A hex literal was expected after 0x:\n";
        print_error_location(state);
        cout << GREEN
    ...
    case BIN_LITERAL_EXPECTED: {
        cout << RED << "\n\r[ERROR " << to_string(BIN_LITERAL_EXPECTED)
             << "] A binary literal was expected after 0b:\n";
        print_error_location(state);
        cout << GREEN
    ...
    case UNTERMINATED_CHARACTER_LITERAL: {
        cout << RED << "\n\r[ERROR "
             << to_string(UNTERMINATED_CHARACTER_LITERAL)
             << "] A character literal was started but never ended:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << UNTERMINATED_CHARACTER_LITERAL_LINK << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;

    case INVALID_COMPILER_DIRECTIVE: {
        cout << RED << "\n\r[ERROR " << to_string(INVALID_COMPILER_DIRECTIVE)
             << "] An invalid compiler directive ("
             << state.tokens[state.tokens.size() - 1].value << ") was found:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << INVALID_COMPILER_DIRECTIVE_LINK << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;
    ...
    case CLOSING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE: {
        cout << RED << "\n\r[ERROR "
             ...
             << "] A closing parenthesis was expected in the @file compiler "
                "directive:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << CLOSING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE_LINK
             << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;
    ...
    case OPENING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE: {
        cout << RED << "\n\r[ERROR "
             ...
             << "] An opening parenthesis was expected in the @file compiler "
                "directive:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << OPENING_PARENTHESIS_EXPECTED_IN_FILE_COMPILER_DIRECTIVE_LINK
             << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;
...
void throw_error(int err_code, Parser state) {
    ...
    case EXPECTED_IDENTIFIER_FOR_FIRST_CLASS_EXPRESSION: {
        cout << RED << "\n\r[ERROR "
             << to_string(EXPECTED_IDENTIFIER_FOR_FIRST_CLASS_EXPRESSION)
             << "] An identifier is expected in a first class expression:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << EXPECTED_IDENTIFIER_FOR_FIRST_CLASS_EXPRESSION_LINK << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;
    case EXPECTED_VARIABLE_NAME_AFTER_TYPE: {
        cout << RED << "\n\r[ERROR "
             << to_string(EXPECTED_VARIABLE_NAME_AFTER_TYPE)
             << "] A name was expected after a type in the "
                "definition/declaration "
                "of a variable:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
             << EXPECTED_VARIABLE_NAME_AFTER_TYPE_LINK << "\n"
             << DEFAULT << endl;
        exit(-1);
    } break;
    case EXPECTED_BOUND_OR_ARRAY_DEFINITION_OR_ASSIGNMENT_AFTER_VARIABLE_NAME: {
        cout
            ...
            << "] A boundary ({}), array ([]), or assignment (=) was expected "
               "after "
    ...
    case EXPECTED_NEWLINE_BEFORE_NEW_FIRST_CLASS_EXPRESSION: {
        cout << RED << "\n\r[ERROR "
             << to_string(EXPECTED_NEWLINE_BEFORE_NEW_FIRST_CLASS_EXPRESSION)
             << "] A new line was expected after the end of a first class "
                "expression:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
    ...
    case EXPEXTED_EXPRESSION_AFTER_OPERATION_IN_ASSIGNMENT: {
        cout << RED << "\n\r[ERROR "
             << to_string(EXPEXTED_EXPRESSION_AFTER_OPERATION_IN_ASSIGNMENT)
             << "] An Expression was expected after an operation in an "
                "assignment:\n";
        print_error_location(state);
        cout << GREEN << "\nRead more about this error: "
...
void throw_type_error(int err_code, Type expected, Type was, Parser state) {
    switch (err_code) {
        case BASE_TYPES_NOT_CONSISTENT: {
            cout << RED << "\r\n [ERROR " << to_string(BASE_TYPES_NOT_CONSISTENT) << "] Types " << type_to_token_value[expected.base_type] << "(" << to_string(expected.dimensions.first) << "x" << to_string(expected.dimensions.second) << ") and " << type_to_token_value[was.base_type] << "(" << to_string(was.dimensions.first) << "x" << to_string(was.dimensions.second) << ") are not consistent! (one canot unexplicitly be casted into the other):\n";
            print_error_location(state);
            cout << GREEN << "\nRead more about this error: " << BASE_TYPES_NOT_CONSISTENT_LINK << "\n" << DEFAULT << endl;
        } break;
    }
}
